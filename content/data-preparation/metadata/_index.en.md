...
* 📄 [**`inclusion_criteria.json`**](https://github.com/metapsy-project/data-template/blob/master/metadata/inclusion_criteria.json): A [`.json`](https://www.json.org/json-en.html) file that documents the database inclusion and exclusion criteria using the [PICO](https://archive.fo/Wom0X) schema. The file **must** include the following six domains under these **exact** names: `population`, `intervention`, `comparison`, `outcome`, `study_design`, and `other`. Each domain is an object with an `inclusion` and an `exclusion` array. Multiple criteria can be listed for each domain. If a domain has no inclusion or exclusion criterion, you can use an empty list (`[]`).
